...
class AccountAPI {
  ...
  Future<User> getUserInfo() async {
    ...
    final response = await _http.response<User>('/api/v1/user-info',
        method: 'GET',
  ...
  Future<String> updateAvatar(Uint8List bytes, String fileName) async {
    ...
    final response = await _http.response<String>(
      '/api/v1/update-avatar',
      method: 'POST', 
      headers: {'token': token!},
      formData: { 'attachment' : MultipartFile.fromBytes(bytes, filename: fileName)}
      );
